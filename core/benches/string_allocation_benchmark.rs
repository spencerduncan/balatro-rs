...
fn bench_format_cache_key(c: &mut Criterion) {
    ...
    group.bench_function("optimized_reuse_string", |b| {
        ...
        b.iter(|| {
            ...
            write!(reusable_string, "hand_{hash_value:x}").unwrap();
...
fn bench_joker_messages(c: &mut Criterion) {
    ...
    group.bench_function("current_format", |b| {
        b.iter(|| {
            ...
            black_box(format!("Seltzer: {hands_remaining} hands remaining"))
        })
    });
    ...
    group.bench_function("optimized_write", |b| {
        b.iter(|| {
            ...
            let mut cursor = std::io::Cursor::new(&mut buffer[..]);
            write!(cursor, "Seltzer: {hands_remaining} hands remaining").unwrap();
    ...
    group.bench_function("optimized_1000x", |b| {
        b.iter(|| {
            ...
            for i in 0..1000 {
                ...
                use std::fmt::Write as FmtWrite;
                write!(cache_key_buffer, "hand_{hash_value:x}").unwrap();
