...
fn test_game_engine_processes_joker_effects() {
    ...
    assert_eq!(score_with_jokers, 80.0);
...
fn test_joker_lifecycle_events() {
    ...
    assert!(game.money >= 5.0); // TestLifecycleJoker gives +5 money on blind start
}
...
fn test_joker_effect_modifiers() {
    ...
    // The TestMultiplierJoker gives +10 mult with a 2.0 mult_multiplier
