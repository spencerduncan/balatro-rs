...
fn test_target_validation_joker_slots() {
    ...
    match target.validate(&game) {
        Err(TargetValidationError::JokerSlotInvalid { slot: 0, joker_count: 0 }) => {}, // Expected
    ...
    match target.validate(&game) {
        Err(TargetValidationError::JokerSlotInvalid { slot: 5, joker_count: 0 }) => {}, // Expected
        other => panic!("Expected JokerSlotInvalid error, got: {:?}", other),
    }
}
...
fn test_target_validation_always_valid_targets() {
    ...
    for target in always_valid_targets {
        ...
        assert!(target.validate(&game).is_ok(), "Target {:?} validation should succeed", target);
    }
}
...
fn test_get_available_targets_no_cards() {
    ...
    assert!(targets.is_empty(), "Should have no available card targets with empty hand");
    
    let targets = Target::get_available_targets(TargetType::Cards(2), &game);
    assert!(targets.is_empty(), "Should have no available multi-card targets with empty hand");
}
...
fn test_get_available_targets_no_jokers() {
    ...
    assert!(targets.is_empty(), "Should have no available joker targets with no jokers");
}
...
fn test_get_available_targets_always_available() {
    ...
    for target_type in always_available {
        ...
        assert!(!targets.is_empty(), "Target type {:?} should always have available targets", target_type);
    }
}
...
fn test_multi_card_combination_generation() {
    ...
    assert!(targets.is_empty(), "Requesting too many cards should return empty");
}
...
fn test_target_validation_error_display() {
    ...
    let errors = vec![
        TargetValidationError::CardIndexOutOfBounds { index: 5, hand_size: 3 },
        TargetValidationError::JokerSlotInvalid { slot: 2, joker_count: 1 },
        TargetValidationError::HandTypeNotAvailable { hand_type: HandRank::Flush },
