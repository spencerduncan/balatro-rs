...
fn test_uniform_distribution_fairness() {
    ...
    for (i, &count) in counts.iter().enumerate() {
        ...
        assert!(
            ...
            i, count, expected_count, deviation, tolerance
        );
    }
}
...
fn test_shuffle_permutation_coverage() {
    ...
    assert!(
        ...
        permutation_counts.len(), expected_permutations, min_expected_unique
    );
    ...
    assert!(
        *max_count <= max_acceptable,
        ...
        max_count, expected_avg, max_acceptable
    );
}
...
fn test_weighted_choice_distribution() {
    ...
    for _ in 0..sample_size {
        let choice = rng.choose_weighted(&items, |i| weights[items.iter().position(|&x| x == *i).unwrap()]).unwrap();
    ...
    for (i, &item) in items.iter().enumerate() {
        ...
        assert!(
            ...
            item, expected_prop, observed_prop, tolerance
        );
    }
}
...
fn test_deterministic_reproducibility() {
    ...
    assert_eq!(sequence1, sequence2, "Deterministic RNG with same seed produced different sequences");
}
...
fn test_seed_independence() {
    ...
    // Count differences
    let differences = sequence1.iter().zip(sequence2.iter()).filter(|(a, b)| a != b).count();
    ...
    assert!(
        ...
        differences, sample_size, min_expected_differences
    );
}
...
fn test_fork_independence() {
    ...
    // Count differences
    let differences = parent_sequence.iter().zip(child_sequence.iter()).filter(|(a, b)| a != b).count();
    let min_expected_differences = sample_size * 90 / 100; // At least 90% should be different
...
fn test_secure_rng_unpredictability() {
    ...
    // Count similarities
    let similarities = sequence1.iter().zip(sequence2.iter()).filter(|(a, b)| a == b).count();
...
fn test_thread_safety() {
    ...
    for thread_id in 0..4 {
        let thread_count = final_results.iter().filter(|(id, _)| *id == thread_id).count();
        assert_eq!(thread_count, 1000, "Thread {} should have contributed 1000 results", thread_id);
    }
}