...
mod optimization_tests {
    ...
    fn test_optimization_performance() {
        ...
        let test_joker_state_manager = std::sync::Arc::new(balatro_rs::joker_state::JokerStateManager::new());
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            processor.process_hand_effects_optimized(&jokers, &mut game_context, &hand, &test_stage);
        }
        ...
        println!("Speedup: {:.2}x", legacy_time.as_nanos() as f64 / optimized_time.as_nanos() as f64);
        println!("\nOptimization metrics:");
        println!("- Gameplay optimized count: {}", metrics.gameplay_optimized_count);
        println!("- Optimization ratio: {:.2}%", metrics.optimization_ratio() * 100.0);
