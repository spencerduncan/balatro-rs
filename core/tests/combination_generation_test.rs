...
fn test_single_card_target_generation() {
    ...
    assert!(targets.len() <= 5, "Should not exceed number of available cards");
}
...
fn test_combination_edge_cases() {
    ...
    assert!(targets.is_empty(), "Requesting more cards than available should return empty");
...
fn test_performance_limits() {
    ...
    assert!(targets.is_empty(), "Should limit combinations for performance (> 5 cards)");
    ...
    for target in targets {
        if let Target::Cards(indices) = target {
            ...
            for &index in &indices {
                assert!(index < 4, "Card index {} should be less than hand size", index);
            }
            ...
            for i in 1..indices.len() {
                assert!(indices[i] > indices[i-1], "Indices should be sorted and unique");
            }
        } else {
...
fn test_mathematical_combination_counts() {
    ...
    let test_cases = vec![
        TestCase { total_cards: 1, cards_to_choose: 1, expected_combinations: 1 },
        TestCase { total_cards: 2, cards_to_choose: 1, expected_combinations: 2 },
        TestCase { total_cards: 2, cards_to_choose: 2, expected_combinations: 1 },
        TestCase { total_cards: 3, cards_to_choose: 1, expected_combinations: 3 },
        TestCase { total_cards: 3, cards_to_choose: 2, expected_combinations: 3 },
        TestCase { total_cards: 4, cards_to_choose: 2, expected_combinations: 6 },
        TestCase { total_cards: 5, cards_to_choose: 2, expected_combinations: 10 },
        TestCase { total_cards: 5, cards_to_choose: 3, expected_combinations: 10 },
    ];
    
    for test_case in test_cases {
        let game = create_game_with_cards(test_case.total_cards);
        let targets = Target::get_available_targets(
            TargetType::Cards(test_case.cards_to_choose), 
            &game
        );
        ...
        println!("Test case: {} choose {} = {} combinations", 
            test_case.total_cards, 
            test_case.cards_to_choose, 
            test_case.expected_combinations);
    }
}
