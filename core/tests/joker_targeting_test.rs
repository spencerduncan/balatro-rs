...
fn test_joker_target_validate_empty_slot() {
    ...
    assert!(matches!(result.unwrap_err(), JokerTargetError::EmptySlot { slot: 3 }));
}
...
fn test_joker_target_get_joker_invalid() {
    ...
    let result = target.get_joker(&game);
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), JokerTargetError::EmptySlot { slot: 5 }));
}
...
fn test_joker_target_error_display() {
    ...
    assert!(inactive_joker_error.to_string().contains("Joker at slot 1 is not active"));
...
fn test_target_joker_at_slot() {
    ...
    assert_eq!(target.target_type(), balatro_rs::consumables::TargetType::Joker);
}
...
fn test_target_joker_invalid_slot() {
    ...
    assert!(matches!(validation_result.unwrap_err(), JokerTargetError::EmptySlot { slot: 5 }));
}
...
fn test_joker_target_edge_cases() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), JokerTargetError::EmptySlot { slot: 0 }));
