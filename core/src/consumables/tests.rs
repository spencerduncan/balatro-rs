...
use crate::consumables::{
    CardCollection, CardTarget, ConsumableSlots, JokerTarget, JokerTargetError, Target, TargetType,
...
fn test_card_target_get_cards_hand_not_implemented() {
    ...
    match result.unwrap_err() {
        ...
        error => {
            panic!("Expected CardIndexOutOfBounds error for index 0 in empty hand, got: {error:?}")
        }
    }
}
