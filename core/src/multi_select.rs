...
impl MultiSelectContext {
    ...
    pub fn range_select_cards(
        &mut self,
        start_id: usize,
        end_id: usize,
        available_card_ids: &[usize],
    ) -> Result<(), MultiSelectError> {
        if !self.active {
            return Err(MultiSelectError::NotActive);
        }

        // Find the indices of start and end cards in the available list
        let start_idx = available_card_ids
            .iter()
            .position(|&id| id == start_id)
            .ok_or(MultiSelectError::InvalidRange)?;
        let end_idx = available_card_ids
            .iter()
        ...
        let new_cards: Vec<usize> = cards_in_range
            .into_iter()
...
mod tests {
    ...
    fn test_inactive_operations() {
        ...
        assert_eq!(
            context.select_joker(JokerId::Joker),
            Err(MultiSelectError::NotActive)
        );
    }
