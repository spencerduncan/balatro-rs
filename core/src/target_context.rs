...
use crate::multi_select::{MultiSelectContext, SelectionLimits, MultiSelectError};
...
impl TargetContext {
    ...
    pub fn target_joker(&mut self, joker_id: JokerId) -> Result<TargetingResult, TargetContextError> {
        if !self.multi_select.is_active() {
    ...
    fn handle_range_selection(&mut self, end_card: Card) -> Result<TargetingResult, TargetContextError> {
        ...
        if selected_cards.is_empty() {
            ...
            let start_card = self.available_cards.iter()
            ...
            match self.multi_select.range_select_cards(start_card.id, end_card.id, &available_ids) {
                Ok(_) => {
                    let range_cards = self.get_cards_in_range(*start_card, end_card);
                    Ok(TargetingResult::RangeSelected { start: *start_card, end: end_card, cards: range_cards })
                }
                Err(e) => Err(TargetContextError::MultiSelectError(e)),
            }
        }
    ...
    pub fn get_target_collection(&self) -> TargetCollection {
        ...
        let cards: Vec<Card> = selected_cards.into_iter()
...
pub enum TargetingResult {
    ...
    RangeSelected { start: Card, end: Card, cards: Vec<Card> },
}
...
impl std::fmt::Display for TargetContextError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::CardNotAvailable(card) => write!(f, "Card {} is not available for targeting", card),
            Self::JokerNotAvailable(joker_id) => write!(f, "Joker {:?} is not available for targeting", joker_id),
