...
/// Action validation trait for business rule enforcement
///
/// Validates actions against current game state and business rules
/// before execution. Critical for preventing invalid state transitions
...
pub trait ActionValidator: Send + Sync {
    ...
    /// * `Err(ApplicationError)` with detailed validation failure
    async fn validate_action(&self, action: &Action, game: &Game) -> Result<(), ApplicationError>;

    /// Pre-validate multiple actions in batch for performance
    ///
    /// Useful for optimizing action space generation and reducing
    /// validation overhead in high-throughput scenarios.
    async fn validate_actions(&self, actions: &[Action], game: &Game) -> Result<Vec<bool>, ApplicationError> {
        let mut results = Vec::with_capacity(actions.len());
        for action in actions {
            results.push(self.validate_action(action, game).await.is_ok());
        }
        Ok(results)
    }

    /// Get validation rules metadata for debugging
