...
mod tests {
    ...
    #[test]
    fn session_id_generates_different_uuids() {
        let mut ids = HashSet::new();

        // Generate 1000 session IDs and ensure they're all unique
        for _ in 0..1000 {
            let id = SessionId::new();
            assert!(ids.insert(id), "Generated duplicate SessionId");
        }
    }
    ...
    #[test]
    fn session_id_can_be_created_from_uuid() {
    ...
    #[test]
    fn session_id_default_creates_new_instance() {
        let id1 = SessionId::default();
        let id2 = SessionId::default();

        assert_ne!(id1, id2, "Default should create new unique instances");
    }

    #[test]
    fn session_id_as_str_returns_string() {
        let id = SessionId::new();
        let str_repr = id.as_str();

        assert_eq!(str_repr.len(), 36);
        assert!(Uuid::parse_str(&str_repr).is_ok());
    }

    #[cfg(feature = "serde")]
    #[test]
    fn session_id_can_be_serialized() {
        let id = SessionId::new();
