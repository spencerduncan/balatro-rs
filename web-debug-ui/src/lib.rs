...
pub use domain::{
    GameSession, SessionId, ValidationResult, ActionValidator,
    GameRepository, StateNotifier, DomainError
};

pub use application::{
    ServiceContainer, ApplicationConfig, GameApplicationService,
    SessionManagementService, CreateGameSessionUseCase, ExecuteGameActionUseCase,
    ApplicationError
};

pub use infrastructure::{
    InfrastructureFoundation, InfrastructureConfig, InfrastructureError,
...
pub mod integration {
    ...
    use crate::application::{ServiceContainer, ApplicationError};
    ...
    use crate::application::{
        config::SessionId,
        container::{
            GameRepository, StateNotifier, MetricsCollector, StateChangeEvent, 
            StateChangeCallback, NotificationHealth, StorageHealth
        }
    };
    ...
    impl StateNotifier for DefaultStateNotifier {
        async fn notify_state_change(&self, _session_id: &SessionId, _event: StateChangeEvent) -> Result<(), ApplicationError> {
            // Stub implementation - just log for now
            tracing::debug!("DefaultStateNotifier::notify_state_change called (stub): {:?}", _event);
            Ok(())
        }
        
        async fn register_callback(&self, _callback: std::sync::Arc<dyn StateChangeCallback>) -> Result<(), ApplicationError> {
            // Stub implementation - just log for now
    ...
    impl MetricsCollector for DefaultMetricsCollector {
        async fn increment_counter(&self, _name: &str, _value: u64, _tags: &[(&str, &str)]) {
            tracing::debug!("DefaultMetricsCollector::increment_counter called (stub): {} = {}", _name, _value);
        }
        
        async fn record_gauge(&self, _name: &str, _value: f64, _tags: &[(&str, &str)]) {
            tracing::debug!("DefaultMetricsCollector::record_gauge called (stub): {} = {}", _name, _value);
        }
        
        async fn record_histogram(&self, _name: &str, _value: f64, _tags: &[(&str, &str)]) {
            tracing::debug!("DefaultMetricsCollector::record_histogram called (stub): {} = {}", _name, _value);
        }
        
        async fn record_timing(&self, _name: &str, _duration: std::time::Duration, _tags: &[(&str, &str)]) {
            tracing::debug!("DefaultMetricsCollector::record_timing called (stub): {} = {:?}", _name, _duration);
        }
        
        fn start_timer(&self, name: &str, _tags: &[(&str, &str)]) -> Box<dyn crate::application::container::Timer> {
            tracing::debug!("DefaultMetricsCollector::start_timer called (stub): {}", name);
        ...
        async fn health_check(&self) -> Result<crate::application::container::MetricsHealth, ApplicationError> {
...
mod tests {
    ...
    async fn integration_service_can_initialize() {
        ...
        match result {
            ...
            Err(e) => {
                println!("Integration service initialization failed (expected during development): {}", e);
